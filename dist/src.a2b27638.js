...
function updateNumbers() {
  document.querySelectorAll("#students-table__body .students-table__body-tr").forEach(function (row, index) {
...
function renderTable() {
  var source = document.getElementById('students-table__body-template').innerHTML.trim();
  var template = Handlebars.compile(source);
  studentsGroup.innerHTML = studentsArr.map(function (student) {
    return template(student);
  }).join('');
}
function editElement(id) {
  var elementId = Number(id);
  var student = studentsArr.find(function (s) {
    return s.number === elementId;
  });
  if (!student) {
  ...
  saveButton.onclick = function () {
    var index = studentsArr.findIndex(function (s) {
      return s.number === elementId;
    });
    if (index !== -1) {
      studentsArr[index] = {
...
function deleteElement(id) {
  ...
  if (index !== -1) {
    ...
    console.log("\u0421\u0442\u0443\u0434\u0435\u043D\u0442 \u0437 ID ".concat(elementId, " \u0432\u0438\u0434\u0430\u043B\u0435\u043D\u0438\u0439"));
    renderTable();
  }
  updateNumbers();
  noteForUser();
...
downloadBtn.addEventListener("click", function () {
  if (studentsArr.length > 0) {
    var jsonStudents = JSON.stringify(studentsArr, null, 2);
    downloadJSON("Students.json", jsonStudents);
    try {
      var parseDataStudents = JSON.parse(jsonStudents);
